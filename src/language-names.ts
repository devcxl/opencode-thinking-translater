...
const PLAIN_LANGUAGE_NAMES: Record<string, string> = {
  zh: "中文",
  en: "English",
  fr: "français",
  de: "Deutsch",
 ...
 *
 * @param languageTag - normalizeLanguageTag 规范化后的 BCP-47 标签
 * @returns 该语言的母语名称
 */
export function getLanguageNativeName(languageTag: string): string {
  if (LANGUAGE_NAMES[languageTag]) return LANGUAGE_NAMES[languageTag]

  const plain = languageTag.split("-", 1)[0]?.toLowerCase()
  if (plain && PLAIN_LANGUAGE_NAMES[plain]) return PLAIN_LANGUAGE_NAMES[plain]
 ...
 */
export function getInstructionTemplate(languageTag: string): string {
  if (INSTRUCTION_TEMPLATES[languageTag]) return INSTRUCTION_TEMPLATES[languageTag]
